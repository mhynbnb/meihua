...
def main():
    ...
    def call_lunar():
        ...
        def change_day1(chioces):
            y = int(cbox_year1.get())
            m = int(cbox_mon1.get())
            monl1 = [1, 3, 5, 7, 8, 10, 12]
            monl2 = [4, 6, 9, 11]
            if m in monl1:
                ...
                if is_lyear(y):
                    ...
                    d_li = [str(i) for i in range(1, 29)]
            cbox_day1.configure(values=d_li)

        def entry_print():
            date = entry.get()
            if len(date) > 8:
                date = date[:8]
            date = ''.join(date[:4]) + '-' + (''.join(date[4:6])).lstrip('0') + '-' + (''.join(date[6:8])).lstrip('0')
            try:
        ...
        def print_sel():
            date = cbox_year1.get() + '-' + cbox_mon1.get() + '-' + cbox_day1.get()
            data, lis_out = date_frame.get_calenda(date)
            try:
                for ii in range(9):
                    for iii in range(14):
        ...
        rst_txt.grid(row=3, column=0, sticky='nw', pady=10, padx=10)
        fram_time1 = ctk.CTkFrame(child, width=400)

        cbox_year1 = ctk.CTkComboBox(fram_time1, width=80, values=yyyy, command=change_day1)
        cbox_year1.grid(row=1, column=0)

        label_year1 = ctk.CTkLabel(fram_time1, text='年', font=('黑体', 13), width=20, height=1)
        label_year1.grid(row=1, column=1)

        cbox_mon1 = ctk.CTkComboBox(fram_time1, width=50, values=mm, command=change_day1)
        cbox_mon1.grid(row=1, column=2)

        label_mon1 = Label(fram_time1, text='月', font=('黑体', 13), width=20, height=1)
        label_mon1.grid(row=1, column=3)

        cbox_day1 = Combobox(fram_time1, width=70, values=dd)
        cbox_day1.grid(row=1, column=4)

        label_day1 = Label(fram_time1, text='日', font=('黑体', 13), width=20, height=1, )
        label_day1.grid(row=1, column=5)
        fram_time1.grid(row=0, column=0, padx=10, pady=10, sticky='w')
        ...
        combo = Combobox(child,
                         values=['黄道吉日', '嫁娶', '入宅', '出行', '宴会', '立券', '搬家', '交易', '求财', '动土', '安床', '动工', '赴任', '安葬',
                                 ...
                                 '诸事宜', '诸事忌', '黑道日'], width=100, command=suitAndNot)
        combo.grid(row=2, column=0, sticky='w', pady=10, padx=10)
        ...
        entry = ctk.CTkEntry(fram_btn2, placeholder_text='例如：20220105')
        entry.grid(row=0, column=0)
        bt1 = ctk.CTkButton(fram_btn2, text="选择", command=lambda: thread_it(print_sel), width=80)
        bt1.grid(row=0, column=1, padx=10)
        bt2 = ctk.CTkButton(fram_btn2, text="输入", command=lambda: thread_it(entry_print), width=80)
